...
func NewCollector(config config.Config) *Collector {
	...
	return &Collector{
		...
		printerTemp:               prometheus.NewDesc("prusa_temperature_celsius", "Current temp of printer in Celsius", append(commonLabels, "printer_heated_element"), nil),
		printerTempTarget:         prometheus.NewDesc("prusa_temperature_target_celsius", "Target temp of printer in Celsius", append(commonLabels, "printer_heated_element"), nil),
		printerPrintTimeRemaining: prometheus.NewDesc("prusa_printing_time_remaining_seconds", "Returns time that remains for completion of current print", commonLabels, nil),
		printerPrintProgressRatio: prometheus.NewDesc("prusa_printing_progress_ratio", "Returns information about completion of current print in ratio (0.0-1.0)", commonLabels, nil),
		printerFiles:              prometheus.NewDesc("prusa_files_count", "Number of files in storage", append(commonLabels, "printer_storage"), nil),
		printerMaterial:           prometheus.NewDesc("prusa_material_info", "Returns information about loaded filament. Returns 0 if there is no loaded filament", append(commonLabels, "printer_filament"), nil),
		printerPrintTime:          prometheus.NewDesc("prusa_print_time_seconds", "Returns information about current print time.", commonLabels, nil),
		...
		printerNozzleSize:         prometheus.NewDesc("prusa_nozzle_size_meters", "Returns information about selected nozzle size.", commonLabels, nil),
		printerStatus:             prometheus.NewDesc("prusa_status_info", "Returns information status of printer.", append(commonLabels, "printer_state"), nil),
		printerAxis:               prometheus.NewDesc("prusa_axis", "Returns information about position of axis.", append(commonLabels, "printer_axis"), nil),
		printerFlow:               prometheus.NewDesc("prusa_print_flow_ratio", "Returns information about of filament flow in ratio (0.0 - 1.0).", commonLabels, nil),
		printerInfo:               prometheus.NewDesc("prusa_info", "Returns information about printer.", append(commonLabels, "api_version", "server_version", "version_text", "prusalink_name", "printer_location", "serial_number", "printer_hostname"), nil),
		printerMMU:                prometheus.NewDesc("prusa_mmu", "Returns information if MMU is enabled.", commonLabels, nil),
		printerFanSpeedRpm:        prometheus.NewDesc("prusa_fan_speed_rpm", "Returns information about speed of hotend fan in rpm.", append(commonLabels, "fan"), nil),
		printerPrintSpeedRatio:    prometheus.NewDesc("prusa_print_speed_ratio", "Current setting of printer speed in values from 0.0 - 1.0", commonLabels, nil),
		printerJobImage:           prometheus.NewDesc("prusa_job_image", "Returns information about image of current print job.", append(commonLabels, "printer_job_image"), nil),
	}
}
